...
type ImportTableRowProps = {
  row: ImportRow
  matchedRule: ImportRule | undefined
  onRowChange: (id: string, patch: Partial<ImportRow>) => void
  onToggleSelected: (id: string) => void
  categoriesBySection: Record<string, BudgetCategory[]>
  suggestions: TxSuggestion[]
}
    ...
    >
            ...
            <div className="absolute left-5 top-1/2 -translate-y-1/2 z-50 hidden group-hover/rule:block w-56 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-lg pointer-events-none">
              ...
              <div className="px-3 py-2 space-y-1.5">
                ...
                {matchedRule.categoryId && (
                  <p className="text-xs text-gray-500 dark:text-gray-400">Category set</p>
                )}
                {matchedRule.tags && (
                  <p className="text-xs text-gray-500 dark:text-gray-400">
                    Tags:{' '}
                    <span className="text-gray-700 dark:text-gray-200">{matchedRule.tags}</span>
                  </p>
                )}
                {matchedRule.renameTo && (
                  <p className="text-xs text-gray-500 dark:text-gray-400">
                    Renamed to:{' '}
                    <span className="text-gray-700 dark:text-gray-200">{matchedRule.renameTo}</span>
                  </p>
                )}
              </div>
            </div>
      ...
      <td className="py-1.5 pl-3 pr-4">
        <input
          type="month"
          value={monthValue}
          onChange={(e) => onRowChange(row.id, { monthOverride: e.target.value || null })}
...
export function EnrichedDataTab({
  ...
  // Local state for the search input so keystrokes don't trigger parent re-renders
  ...
  function handleSearchInput(v: string) {
    ...
    searchDebounce.current = setTimeout(() => onSearchChange(v), 200)
  }
  ...
  // Pre-sort categories by section once, not inside every row render
  const categoriesBySection = useMemo(
    () =>
      SECTIONS.reduce(
        (acc, s) => {
          acc[s] = categories
            .filter((c) => c.section === s && c.label.trim())
  ...
  // O(1) rule lookup per row instead of O(n) find
          ...
          <tbody>
            {filteredRows.map((row) => (
              <ImportTableRow
                ...
                row={row}
                matchedRule={
                  appliedRuleIds?.[row.id] ? rulesMap.get(appliedRuleIds[row.id]) : undefined
                }
